...
melody = [NOTE_E2, 8, NOTE_E2, 8, NOTE_E3, 8, NOTE_E2, 8, NOTE_E2, 8, NOTE_D3, 8, NOTE_E2, 8, NOTE_E2, 8,
...
melody = [NOTE_D5,-4, NOTE_E5,-4, NOTE_A4,4,
...
while 1:
    time.sleep(0.2)
    # If button pressed, start song
    if Pyduino.DigitalRead(BUTTON) == Pyduino.HIGH:
        thisNote = 0
        Pyduino.Message(PRINT_MESSAGE_TYPE, [f"Song: {song}"])
        time.sleep(1)

        while thisNote < notes * 2:
            if Pyduino.DigitalRead(BUTTON) == Pyduino.HIGH:
                # If button pressed while playing, stop song
                Pyduino.NoTone(BUZZER)
                time.sleep(1)
                break

            # Basic tone playing code from online
            ...
            Pyduino.Tone(BUZZER, melody[thisNote], noteDuration * 0.9)

            thisNote += 2

            time.sleep(float(noteDuration) / 1000)

            Pyduino.NoTone(BUZZER)
        Pyduino.Message(PRINT_MESSAGE_TYPE, [f"Done..."])